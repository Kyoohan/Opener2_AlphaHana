...
2. 앱 화면 우측 상단의 열쇠 아이콘(`🔑`)을 눌러 `API 키 설정` 다이얼로그를 엽니다.
3. `Gemini API 키`, `Vertex API 키` 입력란에 각각의 키를 입력하고 `저장`을 누르세요. 필요 시 `지우기` 버튼으로 개별 키를 초기화할 수 있습니다.
4. 접근성 설정(톱니바퀴 아이콘)에서는 글자 크기·고대비 모드·포인트 색상만 다룹니다.
